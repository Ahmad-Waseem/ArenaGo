...
class _SearchResultState extends State<SearchResult> {
  ...
  Future<void> _fetchSearchResults() async {
    try {
      final fields = await _fetchFieldsFromArenaName(widget.arenaName);
      final filteredFields = await _filterFieldsByStartTime(fields, widget.startTime);
      final filteredFields2 = await _filterFieldsByMaxPrice(filteredFields, widget.maxPrice);
      final filteredFields3 = await _filterFieldsByDistance(filteredFields2, widget.maxDistance);
      final sortedfilteredFields = await _sortFieldsByDistance(filteredFields3, widget.friendIds);
      setState(() async {
        _filteredFields = sortedfilteredFields;
  ...
  Future<List<Map<dynamic, dynamic>>> _fetchFieldsFromArenaName(String? arenaName) async {
    List<Map<dynamic, dynamic>> fields = [];

    if (arenaName != null) {

      final arenaSnapshot = await _database.ref('ArenaInfo').orderByChild('arena_name').equalTo(arenaName).get();
      
      if (arenaSnapshot.exists) {
        final Map<dynamic, dynamic>? arenaData = arenaSnapshot.value as Map<dynamic, dynamic>?;

        if (arenaData != null) {
          final List<String> arenaIds = (arenaData.keys.toList()).cast<String>();

          // Fetch fields
          for (String arenaId in arenaIds) {
            final fieldSnapshot = await _database.ref('FieldInfo').orderByChild('arena_id').equalTo(arenaId).get();

            if (fieldSnapshot.exists) {
              final Map<dynamic, dynamic>? fieldData = fieldSnapshot.value as Map<dynamic, dynamic>?;
  ...
  Future<List<Map<dynamic, dynamic>>> _filterFieldsByStartTime(List<Map<dynamic, dynamic>> fields, DateTime? startTime) async {
    if (startTime != null) {
      ...
      for (Map<dynamic, dynamic> field in fields) {
        ...
        final bookingSnapshot = await _database.ref('Bookings').orderByChild('fieldId').equalTo(fieldId).get();

        if (bookingSnapshot.exists) {
          final Map<dynamic, dynamic>? bookingData = bookingSnapshot.value as Map<dynamic, dynamic>?;
  ...
  Future<List<Map<dynamic, dynamic>>> _filterFieldsByMaxPrice(List<Map<dynamic, dynamic>> fields, double? maxPrice) async {
    if (maxPrice != null) {
      List<Map<dynamic, dynamic>> filteredFields = [];
  ...
  Future<List<Map<dynamic, dynamic>>> _filterFieldsByDistance(List<Map<dynamic, dynamic>> fields, double? maxDistance) async {
    ...
    if (maxDistance != null) {
      ...
      for (Map<dynamic, dynamic> field in fields) {
        ...
        if (arenaId != null) {
          final arenaSnapshot = await _database.ref('ArenaInfo/$arenaId/location').get();
          final Map<dynamic, dynamic>? arenaLocation = arenaSnapshot.value as Map<dynamic, dynamic>?;
          ...
          if (arenaLocation != null) {
            ...
            final double arenaLongitude = arenaLocation['longitude'] as double;
              ...
              final userSnapshot = await _database.ref('users/$userId/location').get();
              final Map<dynamic, dynamic>? userLocation = userSnapshot.value as Map<dynamic, dynamic>?;

              if (userLocation != null) {
                final double userLatitude = userLocation['latitude'] as double;
                final double userLongitude = userLocation['longitude'] as double;

                final double distance = _calculateDistance(arenaLatitude, arenaLongitude, userLatitude, userLongitude);
  ...
  double radians(double degrees) {
    ...
    Future<List<Map<dynamic, dynamic>>> _sortFieldsByDistance(List<Map<dynamic, dynamic>> filteredFields, List<String>? FriendIds) async {
      final FirebaseAuth auth = FirebaseAuth.instance;
      ...
      final currentUserSnapshot = await _database.ref('users/$uid/location').get();
      final Map<dynamic, dynamic>? currentUserLocation = currentUserSnapshot.value as Map<dynamic, dynamic>?;

      if (FriendIds != null) {
        final double currentUserLatitude = currentUserLocation?['latitude'] as double;
        final double currentUserLongitude = currentUserLocation?['longitude'] as double;
        ...
        for (Map<dynamic, dynamic> field in filteredFields) {
          ...
          if (arenaId != null) {
            ...
            if (arenaLocation != null) {
              ...
              double totalDistance = distances.fold(0, (prev, element) => prev + element);
        ...
        filteredFields.sort((a, b) => (a['distance'] as double).compareTo(b['distance'] as double));

        // Return the first 20 entries if the list is longer than 20
        return filteredFields.length > 20 ? filteredFields.sublist(0, 20) : filteredFields;
      } else {
        return filteredFields;
      }
    }
